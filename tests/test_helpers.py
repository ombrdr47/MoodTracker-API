...
def test_percentileofscore(L, S, N, expected):
    assert _percentileofscore(L, S, N) == expected

def test_percentileofscore_errors():
    with pytest.raises(ZeroDivisionError, match="Expected ZeroDivisionError"):
...
def test_check_should_update(input, expected):
    assert _check_should_update(input) == expected

def test_check_should_update_errors():
    with pytest.raises(OverflowError, match="Expected OverflowError"):
